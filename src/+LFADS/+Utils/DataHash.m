...
% This function is modified from the original in the following way(s):
% 
% To allow graceful promotion of integer classes, we modify the handling of
% uint and int classes. Since the class of a variable matters for the hash,
% we need to enable a uint16 to be gracefully expanded to a uint32 when the
% need for larger values than 2^16 aries. So we handle this by hashing
% integers as the smallest class (preserving signed or unsigned) that can
% hold them. For legacy reasons, we won't demote to smaller than 16 bit
% types. This behavior is enabled by Opt.AutoDownsizeIntegers = 16
%
...
%            Default: 'array'.
%         Opt.AutoDownsizeIntegers: See note above.
%            false : maintain original integer class
%            true: downgrade to 16 bit integers if value fits
%
%
% OUTPUT:
...
% This is caught with an error message concerning the recursion limit now.
    ...
    if nArg == 2
       ...
       if isfield(Opt, 'AutoDownsizeIntegers') && ~isempty(Opt.AutoDownsizeIntegers)
           autoDownsizeIntegers = Opt.AutoDownsizeIntegers;
       end
...
function Data = handleDownsizeInteger(Data)
    if isa(Data, 'uint32') || isa(Data, 'uint64')
        if max(Data(:)) <= intmax('uint16')
            Data = uint16(Data);
        elseif max(Data(:)) <= intmax('uint32')
            Data = uint32(Data);
        end
    elseif isa(Data, 'int32') || isa(Data, 'int64')
        if min(Data(:)) >= intmin('int16') && max(Data(:)) <= intmax('int16')
            Data = int16(Data);
        elseif min(Data(:)) >= intmin('int32') && max(Data(:)) < intmax('int32')
            Data = int32(Data);
        end
    end
end

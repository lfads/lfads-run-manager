classdef Run < handle & matlab.mixin.CustomDisplay
    ...
    methods
        ...
        function [alignmentMatrices, alignmentBiases] = prepareAlignmentMatrices(r, seqData)
            ...
            if strcmp(r.params.c_output_dist, 'poisson')
                ...
                for nd = 1:numel(datasetInfo)
                    ...
                    for ncond = 1:numel(conditions)
                        ...
                        for nt = 1:n_trials_this_condition
                            trial = datasetInfo(nd).seq(trials_to_use_this_condition(nt));
                            tmp = reshape(trial.y(:, 1:time_per_trial)', ...
                                [], time_per_trial, size(trial.y,1));
                            binned_data(:, :, nt) = squeeze(tmp)';                  
                        end
            ...
            % apply PCA
            if isfield(r.params, 'c_in_factors_dim')
                inFacDim = r.params.c_in_factors_dim;
            else
                inFacDim = r.params.c_factors_dim;
            end
            try
                keep_pcs = pca(all_data', 'Rows', 'pairwise', 'NumComponents', inFacDim);
            catch
                keep_pcs = pca(all_data', 'Rows', 'complete', 'NumComponents', inFacDim);
            end
